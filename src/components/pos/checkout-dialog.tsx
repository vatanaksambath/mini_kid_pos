...
export default function CheckoutDialog({ total, items, onSuccess }: CheckoutDialogProps) {
  ...
  const pointsValue      = redeemPoints ? Math.min(total, loyaltyPoints * 0.01) : 0 // 1pt = $0.01
  ...
  const handleCheckout = () => {
    ...
    startTransition(async () => {
      const loyaltyPointsRedeemed = redeemPoints ? Math.floor(pointsValue / 0.01) : 0
      ...
      const result = await createOrder({
        ...
        items: editableItems,
        payments: [{ amount: finalTotal, paymentMethod: dbPaymentMethod }],
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[520px] max-h-[92vh] overflow-y-auto">
        {step === 'details' ? (
          <>
            <DialogHeader>
              <DialogTitle className="text-lg font-bold">Finalize Sale</DialogTitle>
            </DialogHeader>
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-1.5 text-sm">
                ...
                <div className="flex justify-between font-bold text-lg pt-1 border-t"><span>Total</span><span className="text-primary">${finalTotal.toFixed(2)}</span></div>
              ...
              <Button onClick={handleCheckout} className="w-full h-12 text-base font-bold" disabled={isPending}>
                {isPending ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" /> Processing...</> : `Confirm Payment · $${finalTotal.toFixed(2)}`}
              </Button>
            </div>
          </>
        ) : (
